...
export default function ChatBox({ user }: { user: AuthUser }) {
    ...
    const selectedIsOwn = selected?.clientId === user.id;
    const conversationName = (conversation?: ChatConversation) => {
        if (!conversation) return "Selecciona un cliente";
    ...
    return <div className={`${isAdmin ? "grid md:grid-cols-[280px_minmax(0,1fr)]" : "flex"} h-[calc(100vh-250px)] min-h-[500px] max-h-[680px] overflow-hidden rounded-2xl border border-white/[.06] bg-[#1C1C1C]`}>
        {isAdmin && <aside className="border-b border-white/[.06] bg-[#151515] md:border-b-0 md:border-r"><div className="border-b border-white/[.06] p-4"><h2 className="font-display text-lg font-semibold">Conversaciones</h2><p className="mt-1 text-xs text-zinc-500">{conversations.length} conversaciones</p></div><div className="max-h-52 overflow-auto md:max-h-[560px]">{conversations.map((conversation) => <button key={conversation.id} onClick={() => setSelectedId(conversation.id)} className="flex w-full items-center gap-3 border-b border-white/[.04] p-4 text-left" style={{ backgroundColor: selectedId === conversation.id ? "rgba(201,169,110,.1)" : "transparent" }}><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#C9A96E] font-semibold text-black">{conversation.clientId === user.id ? user.name?.[0]?.toUpperCase() || "A" : conversation.clientName?.[0]?.toUpperCase() || "C"}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{conversationName(conversation)}</p><p className="truncate text-xs text-zinc-500">{conversation.lastMessage || (conversation.clientId === user.id ? "Cuenta administradora" : conversation.clientEmail)}</p></div>{conversation.unreadCount > 0 && <span className="rounded-full bg-[#C9A96E] px-2 py-0.5 text-xs font-bold text-black">{conversation.unreadCount}</span>}</button>)}</div></aside>}
        <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden"><header className="flex h-16 shrink-0 items-center gap-3 border-b border-white/[.06] px-5"><div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#C9A96E] font-bold text-black">{isAdmin ? selectedIsOwn ? user.name?.[0]?.toUpperCase() || "A" : selected?.clientName?.[0]?.toUpperCase() || "C" : "B"}</div><div><p className="text-sm font-semibold">{isAdmin ? conversationName(selected) : "BarberFlow Studio"}</p><p className="text-xs text-green-400">{selectedIsOwn ? "Conversación personal" : "Chat interno"}</p></div></header>
            <div ref={messagesRef} className="min-h-0 flex-1 space-y-3 overflow-y-scroll overscroll-contain bg-[#111111] p-5">{messages.length === 0 && <p className="py-20 text-center text-sm text-zinc-500">Aún no hay mensajes. Inicia la conversación.</p>}{messages.map((message) => { const mine = message.senderId === user.id; return <div key={message.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}><div className="max-w-[82%] rounded-2xl px-3 py-2 text-sm" style={{ backgroundColor: mine ? "#C9A96E" : "#27272A", color: mine ? "#111" : "#F8F5F0" }}>{message.image && <img src={getImageUrl(message.image)} alt="Imagen enviada" onLoad={() => scrollToLatest()} className="mb-2 max-h-72 max-w-full rounded-xl object-cover" />}{message.content && <p className="whitespace-pre-wrap break-words">{message.content}</p>}<div className="mt-1 flex items-center justify-end gap-1 text-[10px] opacity-65"><span>{new Date(message.timestamp).toLocaleTimeString("es-MX", { hour: "2-digit", minute: "2-digit" })}</span>{mine && !selectedIsOwn && <span>{message.isRead ? "✓✓ Leído" : "✓ Enviado"}</span>}</div></div></div>; })}</div>
