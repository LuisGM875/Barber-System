...
import { AlertTriangle, CalendarDays, CheckCircle2, Clock3, MessageCircle, Pencil, Plus, Trash2, UserRound, X } from "lucide-react";
...
type Section = "home" | "appointments" | "messages" | "profile";
...
function AppointmentRow({ appointment, isAdmin, onConfirm, onStatus, onReschedule }: { appointment: Appointment; isAdmin: boolean; onConfirm: (id: string) => void; onStatus: (id: string, status: "CONFIRMED" | "COMPLETED" | "NO_SHOW") => void; onReschedule: (appointment: Appointment) => void }) {
    ...
    return (
        <article className="flex flex-col gap-4 rounded-2xl border border-white/[.06] bg-[#1C1C1C] p-4 sm:flex-row sm:items-center">
            <img src={`http://localhost:8080${appointment.serviceImage}`} alt={appointment.serviceName} className="h-24 w-full rounded-xl object-cover sm:h-20 sm:w-24" />
...
export default function ClientDashboard() {
    ...
    const activeSection = SECTIONS.some((item) => item.id === requestedSection) ? requestedSection! : "home";
            ...
            <section className="flex-1 bg-[#111111] px-6 pb-20 pt-24">
                <div className="mx-auto max-w-6xl">
                    <div className="mb-8 flex w-fit max-w-full gap-2 overflow-x-auto rounded-2xl border border-white/[.06] bg-[#0D0D0D] p-2">
                        {SECTIONS.map((section) => <button key={section.id} onClick={() => changeSection(section.id)} className="whitespace-nowrap rounded-xl px-4 py-2.5 text-sm" style={{ color: activeSection === section.id ? "#111111" : "#A1A1AA", backgroundColor: activeSection === section.id ? "#C9A96E" : "transparent" }}>{section.label}</button>)}
                    </div>
                    ...
                    {activeSection === "messages" && <div className="animate-fadeIn"><h1 className="font-display text-3xl font-bold">Mensajes</h1><p className="mt-2 text-sm text-zinc-400">Tus conversaciones con la barbería aparecerán aquí.</p><div className="mt-7 rounded-2xl border border-white/[.06] bg-[#1C1C1C] p-4"><div className="flex items-center gap-4"><div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#C9A96E] text-black"><MessageCircle size={22} /></div><div className="flex-1"><h2 className="font-semibold">BarberFlow Studio</h2><p className="mt-1 text-sm text-zinc-400">Conversación preparada · Próximamente podrás enviar mensajes.</p></div><span className="hidden text-xs text-zinc-500 sm:block">Sin mensajes</span></div></div></div>}
