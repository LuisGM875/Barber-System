...
import DateCalendarPicker from "../../appointments/components/dateCalendarPicker";
    ...
    return (
        <MainLayout>
            <section className="flex-1 bg-[#111111] px-6 pb-20 pt-24">
                ...
                {rescheduling && <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 px-4 py-6 backdrop-blur-sm"><div className="max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-3xl border border-white/10 bg-[#171717] shadow-2xl"><div className="flex items-center justify-between border-b border-white/10 px-6 py-5"><div><p className="text-xs uppercase tracking-[.18em] text-[#C9A96E]">Reagendar cita</p><h2 className="mt-1 font-display text-2xl font-semibold">{rescheduling.serviceName}</h2></div><button onClick={() => setRescheduling(null)} className="rounded-full bg-white/5 p-2 text-zinc-400"><X size={18} /></button></div><div className="space-y-5 px-6 py-5"><DateCalendarPicker value={rescheduleDate} min={todayValue} onChange={changeRescheduleDate} />{error && <p className="rounded-xl bg-red-500/10 p-3 text-sm text-red-200">{error}</p>}<div><p className="mb-3 text-xs text-zinc-400">Horario disponible</p>{rescheduleLoading ? <p className="text-sm text-zinc-500">Consultando horarios...</p> : <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">{AVAILABLE_TIMES.map((time) => { const unavailable = bookedTimes.includes(time) || isPastTime(time); return <button key={time} disabled={unavailable} onClick={() => setRescheduleTime(time)} className="rounded-lg border px-2 py-2 text-xs disabled:cursor-not-allowed disabled:opacity-35" style={{ borderColor: rescheduleTime === time ? "#C9A96E" : "rgba(255,255,255,.1)", backgroundColor: rescheduleTime === time ? "#C9A96E" : "#0D0D0D", color: rescheduleTime === time ? "#111" : "#F8F5F0" }}>{unavailable ? "🔒 " : ""}{time}</button>; })}</div>}</div><p className="text-xs text-zinc-500">El horario anterior queda disponible durante esta consulta. El nuevo horario respeta las demás citas registradas.</p></div><div className="sticky bottom-0 flex gap-3 border-t border-white/10 bg-[#171717] px-6 py-4"><button onClick={() => setRescheduling(null)} className="rounded-xl border border-white/10 px-4 py-3 text-sm">Cancelar</button><button disabled={!rescheduleTime || rescheduleLoading} onClick={saveReschedule} className="flex-1 rounded-xl bg-[#C9A96E] px-4 py-3 text-sm font-semibold text-black disabled:opacity-40">Guardar nuevo horario</button></div></div></div>}
